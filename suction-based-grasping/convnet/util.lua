...
-- Pre-process images for ResNet-50 pre-trained on ImageNet (224x224 RGB mean-subtracted)
...
function getTrainingExample(toteImgsPath,objNames,trainObjIdx,numImgs)

    -- Pick random training image
    local randImgIdx = math.floor(math.random()*numImgs[trainObjIdx])
    local randImgPrefix = string.format('img-%06d',randImgIdx)

    local imgPrefix = paths.concat(toteImgsPath,objNames[trainObjIdx],randImgPrefix)

    -- print(imgPrefix..'.color.png')

    -- Load tote image and pre-process image
    local toteImg = image.load(imgPrefix..'.color.png')
    toteImg = preprocessImg(toteImg)

    -- Load segmentation mask
    local segmLabel = image.load(imgPrefix..'.mask.png')
    segmLabel = segmLabel:mul(trainObjIdx)+1
    segmLabel = image.scale(segmLabel,224,224,'simple')
    
    return toteImg,segmLabel
end
